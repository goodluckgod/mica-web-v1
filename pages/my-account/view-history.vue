<template>
  <div>
    ...
    >
      <div>
        <p class="text-xl">Önceden Gezdiklerim</p>
      </div>
      <div class="h-12">
        <div v-if="products.length > 0" @click="clearproducts()">
          <button-error title="Geçmişi Temizle" />
        </div>
      </div>
    </div>
      ...
      <div v-else class="flex items-center justify-center">
        <div class="w-6/12 flex flex-col text-center gap-5">
          <div class="flex items-center justify-center">
            <icon-box class="fill-primary-300 h-24 w-24" />
          </div>
...
export default {
  ...
  methods: {
    ...
    getproducts() {
      this.$axios
        .get("/v1/profile/customer/history/products")
        .then((response) => {
    ...
    clearproducts() {
      this.$axios
        .delete("/v1/profile/customer/history/products/clear")
        .then((response) => {
          if (response.data.status == "success") {
            this.getproducts();
          }
